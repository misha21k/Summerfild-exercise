...
class MainWindow:
    ...
    def fileImport(self, *ignore):
        ...
        db = None
        try:
            db = shelve.open(filename, flag='r', protocol=pickle.HIGHEST_PROTOCOL)
            for name, url in db.items():
                self.data[name] = url
            for name in sorted(self.data, key=str.lower):
                self.listBox.insert(tkinter.END, name)
            self.setStatusBar("Imported {0} bookmarks from {1}".format(
                self.listBox.size(), filename))
        except (EnvironmentError, pickle.PickleError) as err:
            tkinter.messagebox.showwarning("Bookmarks - Error",
                    "Failed to import {0}:\n{1}".format(
                    self.filename, err), parent=self.parent)
    ...
    def fileExport(self, *ignore):
        ...
        if not filename:
            return
        if not filename.endswith('.dbm'):
            filename += '.dbm'
        db = None
        try:
            db = shelve.open(filename, flag='n', protocol=pickle.HIGHEST_PROTOCOL)
            db.update(self.data)
            db.sync()
            self.setStatusBar("Exported {0} bookmarks from {1}".format(
                self.listBox.size(), filename))
        except (EnvironmentError, pickle.PickleError) as err:
            tkinter.messagebox.showwarning("Bookmarks - Error",
                                           "Failed to export {0}:\n{1}".format(
